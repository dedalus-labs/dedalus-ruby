...
class Dedalus::Test::SingletonClient < Dedalus::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Dedalus::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Dedalus::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @dedalus = Dedalus::Test::SingletonClient.instance
  end
